...
class WaveFunction(ABC):
    ...
    def update_wave_function_and_propagate(self, cell: Cell, collapsed_cell: Cell) -> 'WaveFunction':
        new_wave_function = self._update_wave_function(cell, collapsed_cell)
        new_wave_function = new_wave_function._propagate(collapsed_cell)
    ...
    def _propagate(self, cell: Cell, visited: list[Cell] = None) -> 'WaveFunction':
        if visited is None:
            visited = []

        current_wave_function = self

        influenced_cells = current_wave_function._get_influenced_cells(cell)
        ...
        for influenced_cell in influenced_cells_except_already_visited:
            ...
            if pruned_cell is not influenced_cell:
                ...
                current_wave_function = current_wave_function._update_wave_function(influenced_cell, pruned_cell)

        for pruned_cell in pruned_cells:
            current_wave_function = current_wave_function._propagate(pruned_cell, [*visited, cell])
